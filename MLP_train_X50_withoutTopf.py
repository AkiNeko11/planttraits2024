...
# 创建模型保存目录
os.makedirs('X50_model_MLP_withoutTopf', exist_ok=True)
...
def load_and_preprocess_data():
    ...
    # 保存统计信息
    with open('X50_model_MLP_withoutTopf/stats.json', 'w') as f:
        json.dump(stats, f, indent=2)

    print("统计信息已保存到 X50_model_MLP_withoutTopf/stats.json")
...
def train_model(X, y, y_original, stats, epochs=1000, batch_size=64):
    ...
    for epoch in range(epochs):
        ...
        if val_r2 > best_val_r2:
            best_val_r2 = val_r2
            torch.save(model.state_dict(), 'X50_model_MLP_withoutTopf/model.pt')
...
def main():
    ...
    print("\n=== 训练完成 ===")
    print(f"模型文件: X50_model_MLP_withoutTopf/model.pt")
    print(f"统计信息: X50_model_MLP_withoutTopf/stats.json")
    print(f"最终R²: {r2:.6f}")
