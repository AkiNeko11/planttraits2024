...
# 创建模型保存目录
os.makedirs('X11_model_MLP_Topf', exist_ok=True)
...
def get_top20_features():
    """获取top20特征列表（仅按第一列读取）"""
    # 直接读取第一列（第一列没有列名），使用 header=None 防止 pandas 自动处理
    df = pd.read_csv('X11_top20f/X11_feature_importance_analysis.csv', header=None)

    # 提取第一列的前 20 个（跳过第一行，即从第2行开始取20个）
    feature_names = df.iloc[1:21, 0].tolist()
...
def load_and_preprocess_data():
    ...
    # 获取top20特征
    top20_features = get_top20_features()
    ...
    # 保存统计信息
    with open('X11_model_MLP_Topf/stats.json', 'w') as f:
        json.dump(stats, f, indent=2)

    print("统计信息已保存到 X11_model_MLP_Topf/stats.json")
...
def train_model(X, y, y_original, stats, epochs=1000, batch_size=64):
    ...
    for epoch in range(epochs):
        ...
        if val_r2 > best_val_r2:
            best_val_r2 = val_r2
            torch.save(model.state_dict(), 'X11_model_MLP_Topf/model.pt')
...
def main():
    ...
    print("\n=== 训练完成 ===")
    print(f"模型文件: X11_model_MLP_Topf/model.pt")
    print(f"统计信息: X11_model_MLP_Topf/stats.json")
    print(f"最终R²: {r2:.6f}")
