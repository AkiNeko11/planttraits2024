...
# 创建预测结果保存目录
os.makedirs('X26_MLP_predictions', exist_ok=True)
...
def load_model_and_stats():
    ...
    # 加载统计信息
    with open('X26_model_MLP_withoutTopf/stats.json', 'r') as f:
        stats = json.load(f)
...
def save_predictions(predictions, ids, stats):
    ...
    results_df = pd.DataFrame({
        'id': ids,
        'X26_mean': predictions
    })
    
    # 保存到CSV文件
    output_file = 'X26_MLP_predictions/X26_MLP_predictions.csv'
    ...
    # 保存预测统计信息
    stats_file = 'X26_MLP_predictions/prediction_stats.json'
    prediction_stats = {
...
def main():
    ...
    # 加载训练好的权重
    model.load_state_dict(torch.load('X26_model_MLP_withoutTopf/model.pt', map_location=device))
    print("模型权重已加载")
    ...
    if predictions is not None:
        ...
        print("\n=== 预测完成 ===")
        print(f"预测结果文件: X26_MLP_predictions/X26_MLP_predictions.csv")
        print(f"预测统计文件: X26_MLP_predictions/prediction_stats.json")
        
        # 显示前几个预测结果
        print("\n前5个预测结果:")
        for i in range(min(5, len(predictions))):
            print(f"ID: {test_ids[i]}, X26_mean: {predictions[i]:.6f}")
    else:
        print("预测失败！")
