...
# 创建输出文件夹
os.makedirs('X11_model_results', exist_ok=True)
...
for name, model in models.items():
    with open(f'X11_model_results/{name}_model.pkl', 'wb') as f:
        pickle.dump(model, f)
    print(f"{name} 模型已保存到: X11_model_results/{name}_model.pkl")

# 保存预处理器
with open('X11_model_results/target_scaler.pkl', 'wb') as f:
    pickle.dump(y_scaler, f)
with open('X11_model_results/feature_scaler.pkl', 'wb') as f:
    pickle.dump(feature_scaler, f)
print("预处理器已保存")

# 保存特征列名
with open('X11_model_results/feature_cols.json', 'w', encoding='utf-8') as f:
...
with open('X11_model_results/transform_info.json', 'w', encoding='utf-8') as f:
    json.dump(transform_info, f, indent=2, ensure_ascii=False)
print("变换信息已保存")

# 保存验证集评估结果
with open('X11_model_results/validation_results.json', 'w', encoding='utf-8') as f:
...
# 保存验证集预测结果
validation_df.to_csv('X11_model_results/validation_predictions.csv', index=False)
print("验证集预测结果已保存到: X11_model_results/validation_predictions.csv")
...
validation_stats.to_csv('X11_model_results/validation_statistics.csv', index=False)
print("验证集统计结果已保存到: X11_model_results/validation_statistics.csv")
...
plt.tight_layout()
plt.savefig('X11_model_results/validation_performance.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n=== 训练完成 ===")
print(f"所有模型和结果已保存到 X11_model_results 文件夹")
