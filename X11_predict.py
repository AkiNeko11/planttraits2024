...
# === 检查模型文件是否存在 ===
model_dir = 'X11_model_results'
if not os.path.exists(model_dir):
    print(f"错误: 模型文件夹 {model_dir} 不存在!")
    print("请先运行 X11_train_model.py 训练模型")
    exit()
...
# 创建输出文件夹
os.makedirs('X11_predictions', exist_ok=True)
...
for name, pred in predictions.items():
    ...
    predictions_df.to_csv(f'X11_predictions/{name}_predictions.csv', index=False)
    print(f"{name} 预测结果已保存到: X11_predictions/{name}_predictions.csv")
...
all_predictions_df.to_csv('X11_predictions/all_models_predictions.csv', index=False)
print("综合预测结果已保存到: X11_predictions/all_models_predictions.csv")

if has_target:
    # 保存测试集评估结果
    with open('X11_predictions/test_results.json', 'w', encoding='utf-8') as f:
        json.dump(test_results, f, indent=2, ensure_ascii=False)
    print("测试集评估结果已保存到: X11_predictions/test_results.json")
...
if has_target:
    ...
    plt.tight_layout()
    plt.savefig('X11_predictions/model_performance_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('X11_predictions/prediction_vs_actual.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('X11_predictions/prediction_distributions.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('X11_predictions/model_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()

print("\n=== 预测完成 ===")
print(f"所有结果已保存到 X11_predictions 文件夹")
